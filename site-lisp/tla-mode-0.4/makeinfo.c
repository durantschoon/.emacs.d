...
#define SPLIT_SIZE_THRESHOLD 64000  /* What's good enough for Stallman... */
     ...
     char **argv;
{
  ...
  /* If the user has specified no-headers, that implies no-split.
     Make it so. */
  if (no_headers)
    {
      splitting = 0;

      /* You know, I have the feeling that in this case we should always
	 output using COMMAND_OUTPUT_FILENAME, and that if it isn't set,
	 we should make it be `-'.  However, this might break something
	 that the FSF distributes, so I won't do it without giving fair
	 warning. */
#if 0
      if (!command_output_filename)
	command_output_filename = savestring ("-");
#endif
    }
