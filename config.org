...
** Package management
*** add marmalade repo for additional packages (marmalade is currently down, so disable for now)
...
*** Git
    ...
    Actually this requires setting up a prefix-key properly which I'll have to
    learn about later.
